      ...
      <div className="flex bg-gray-100 rounded-xl p-1 mb-5">
        {[
          ...
          <button key={f.key} onClick={() => setFilter(f.key)}
            className={cn('flex-1 py-2 rounded-lg text-sm font-semibold text-center min-h-tap transition-all duration-200',
              filter === f.key ? 'bg-white text-gray-900 shadow-card' : 'text-gray-500')}>
            {f.label}
          </button>
        ))}
      </div>
...
export default function Route() {
  ...
  return (
    <>
      <Header title="Route" right={
        <button
          onClick={() => setActiveTab(activeTab === 'route' ? 'calendar' : 'route')}

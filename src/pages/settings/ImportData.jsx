    ...
    <>
      ...
      <PageWrapper>
        <div className="space-y-5">
            ...
            {IMPORT_TYPES.map(t => (
              <button
                ...
                className={cn('flex-1 py-2.5 text-sm font-semibold text-center rounded-lg min-h-tap transition-all',
                  importType === t.value ? 'bg-white text-gray-900 shadow-card' : 'text-gray-500')}
          ...
          {/* Upload area */}
          <Card>
            <h3 className="text-sm font-bold text-gray-900 mb-2">Upload CSV File</h3>
            <p className="text-xs text-gray-400 mb-3">
              Expected columns: {typeDef.columns.map(c => (
                <span key={c} className={cn('font-mono', typeDef.required.includes(c) && 'font-bold text-gray-600')}>
            ...
            <button
              ...
              className="w-full flex items-center justify-center gap-2 py-4 border-2 border-dashed border-gray-200 rounded-xl hover:border-pool-400 hover:bg-pool-50/30 transition-all cursor-pointer"
            >
            ...
            <Card>
              ...
              <div className="overflow-x-auto -mx-4 px-4">
                <table className="w-full text-[11px] border-collapse">
                  <thead>
                    <tr className="bg-gray-50">
                      {parsed.headers.map((h, i) => (
                        <th key={i} className="px-2 py-1.5 text-left font-semibold text-gray-600 border-b">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {parsed.rows.slice(0, 5).map((row, i) => (
                      <tr key={i} className="border-b border-gray-100">
                        {row.map((cell, j) => (
                          <td key={j} className="px-2 py-1.5 text-gray-700 truncate max-w-[120px]">{cell}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
                ...
                {parsed.rows.length > 5 && (
                  <p className="text-[11px] text-gray-400 mt-1">...and {parsed.rows.length - 5} more rows</p>
                )}
              </div>
          ...
          {result && (
            <Card className={cn(result.errors.length > 0 ? 'border-l-4 border-l-amber-400' : 'border-l-4 border-l-green-400')}>
              <h3 className="text-sm font-bold text-gray-900 mb-2">Import Complete</h3>
              <div className="flex gap-4 text-sm">
                <div><span className="font-bold text-green-600">{result.imported}</span> imported</div>
                <div><span className="font-bold text-gray-400">{result.skipped}</span> skipped</div>

...
export default function Jobs() {
  ...
  const [jobForm, setJobForm] = useState({ client_id: '', pool_id: '', title: '', scheduled_date: new Date().toISOString().split('T')[0], scheduled_time: '', notes: '', price: '' })
  ...
  async function handleJobSubmit(e) {
    ...
    try {
      ...
      setJobForm({ client_id: '', pool_id: '', title: '', scheduled_date: new Date().toISOString().split('T')[0], scheduled_time: '', notes: '', price: '' })
    } catch (err) {
      ...
      <PageWrapper>
        ...
        <div className="flex gap-2 mb-4">
          ...
          <button onClick={() => navigate('/recurring-jobs')}
            className="flex items-center justify-center gap-1.5 px-3 py-2.5 rounded-xl bg-white border border-gray-200 shadow-card text-xs font-semibold text-gray-600 hover:bg-gray-50 active:scale-[0.98] transition-all min-h-tap">
